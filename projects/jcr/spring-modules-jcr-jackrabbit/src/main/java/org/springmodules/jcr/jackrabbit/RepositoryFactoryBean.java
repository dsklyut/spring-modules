    ...
    protected void resolveConfigurationResource() throws Exception {
        ...
        if (this.configuration == null) {
            if (logger.isDebugEnabled())
                logger.debug("no configuration resource specified, using the default one:" + DEFAULT_CONF_FILE);
            configuration = new ClassPathResource(DEFAULT_CONF_FILE);
        }

        if (homeDir == null) {
            if (logger.isDebugEnabled())
                logger.debug("no repository home dir specified, using the default one:" + DEFAULT_REP_DIR);
